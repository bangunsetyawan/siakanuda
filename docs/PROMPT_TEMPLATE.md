...
2. Buat patch .tar.gz (WAJIB kecualikan folder 'dashboard/writable' agar tidak bentrok permisi www-data di server)
3. Upload via SCP ke [SSH_USER]@[IP_SERVER_TAILSCALE]:~/ (Tailscale) atau [IP_SERVER_LAN]:~/ (LAN)
4. Beri saya command SSH untuk extract & restart service yang relevan
5. Update DEPLOY_LOG.md (lokal & server)

Aturan restart & deployment:
- Ubah file PHP/HTML/CSS (dashboard/) → diproses dinamis oleh Nginx + PHP 8.4 FPM, TIDAK PERLU restart service apa pun (cukup refresh browser).
- Ubah file JS (execution/) → restart bot.siswa.service (sudo systemctl restart bot.siswa).
- JANGAN jalankan/restart siakadash.service karena sudah di-disable permanently (port 8080 di-handle langsung oleh Nginx).
...
Saya sedang SSH ke server Debian ([SSH_USER]@[IP_SERVER_TAILSCALE] / [IP_SERVER_LAN]).

Bantu saya mengecek kondisi server:
1. Status 4 service aktif (bot.siswa, nginx, php8.4-fpm, cloudflared)
2. Log error hari ini (journalctl -u bot.siswa -n 50 --no-pager + CI4 writable/logs)
