...
* **v1.6.5** (4 Jun 2026) - Penyelarasan & Deployment Services Debian: Deployment pembaruan kode SIAKANUDA ke server Debian (IP [IP_SERVER_TAILSCALE]), penyelarasan nama systemd service dari `siswabot.service` menjadi `bot.siswa.service` untuk menyamakan dengan blueprint produksi, pembersihan service lama, pemuatan skema database SQLite `tahun_pelajaran` secara otomatis lewat daemon startup, serta pengaktifan kembali layanan `bot.siswa` dan `siakadash` dengan status active (running).
